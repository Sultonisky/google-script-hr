...
<head>
    ...
    @include('components.seo', [
        'title' => trim($__env->yieldContent('title')) ?: 'MITO HRIS',
        'description' => 'Halaman internal MITO HRIS.',
...
<body>
    ...
    <script>
        ...
        function renderDrawerCandidateBadge(statusLabel, offeringResponse) {
            ...
            if (offeringResponse) {
                var respClass = 'hold',
                    respIcon = 'bi-hourglass-split';
                if (offeringResponse === 'Diterima') {
                    respClass = 'accepted';
                    respIcon = 'bi-check-circle-fill';
                }
                if (offeringResponse === 'Ditolak') {
                    respClass = 'blacklist';
                    respIcon = 'bi-x-circle-fill';
                }
        ...
        window.openCandidateDrawer = function(id) {
            ...
            fetch('/hr/recruitment/' + id + '/json', {
                ...
                .then(function(data) {
                    ...
                    if (metaEl) metaEl.innerHTML = '<span>Recruitment ID<strong>' + (c.recruitmentId || '-') +
                        '</strong></span><span>Tanggal Daftar<strong>' + (c.createdDate || '-') +
                        '</strong></span>';
                    ...
                    if (respBtn) respBtn.onclick = function() {
                        window.openOfferingResponseModal(c);
                    };
                    ...
                    if (tlWrap) {
                        ...
                        if (!logs.length) {
                            ...
                            var iconMap = {
                                ...
                                'HR Notes Update': 'bi-chat-square-text-fill'
                            };
                            var dotMap = {
                                'Created': 'created',
                                'Accepted': 'accepted',
                            ...
                            tlWrap.innerHTML = logs.map(function(ev) {
                                ...
                                return '<div class="timeline-item">' +
                                    '<div class="timeline-dot ' + dot + '"><i class="bi ' + icon +
                                    '"></i></div>' +
                                    '<div class="timeline-title">' + action + '</div>' +
                                    '<div class="timeline-desc">' + oldValue + ' → ' + newValue + '</div>' +
                                    '<div class="timeline-time"><i class="bi bi-clock"></i> ' + timestamp +
                                    ' · ' + user + '</div>' +
                                    '</div>';
                            }).join('');
                        }
                ...
                .catch(function() {
                    renderEntityTimeline('empDrTimeline', []);
        ...
        function renderEntityNotes(elementId, generalNotes, hrNotes) {
            var el = document.getElementById(elementId);
            if (!el) return;
            var sections = [];
            if (generalNotes) sections.push('<strong>Umum:</strong>\n' + escapeDrawerHtml(generalNotes));
            if (hrNotes) sections.push('<strong>HR:</strong>\n' + escapeDrawerHtml(hrNotes));
        ...
        function renderEntityTimeline(elementId, auditLogs) {
            ...
            if (!Array.isArray(auditLogs) || !auditLogs.length) {
                wrap.innerHTML = '<div class="timeline-empty">Belum ada riwayat aktivitas.</div>';
                return;
            }
            var iconMap = {
                Created: 'bi-plus-circle-fill',
            ...
            wrap.innerHTML = auditLogs.map(function(ev) {
                ...
                var field = ev.Field || ev.field || '';
                var oldValue = ev['Old Value'] || ev.oldValue || '';
                var newValue = ev['New Value'] || ev.newValue || '';
                var timestamp = ev.Timestamp || ev.timestamp || '';
                var user = ev.User || ev.user || '';
                var source = ev.Source || ev.source || '';
                var detail = oldValue || newValue ? escapeDrawerHtml(oldValue || '-') + ' &rarr; ' +
                    escapeDrawerHtml(newValue || '-') : '';
                return '<div class="timeline-item"><div class="timeline-dot pending"><i class="bi ' + (iconMap[
                        action] || 'bi-dot') + '"></i></div>' +
                    '<div class="timeline-title">' + escapeDrawerHtml(field ? action + ' · ' + field : action) +
                    '</div>' +
                    (detail ? '<div class="timeline-desc">' + detail + '</div>' : '') +
                    '<div class="timeline-time"><i class="bi bi-clock"></i> ' + escapeDrawerHtml(timestamp) +
                    (user ? ' · Oleh: ' + escapeDrawerHtml(user) : '') + (source ? ' · ' + escapeDrawerHtml(
                        source) : '') + '</div></div>';
            }).join('');
        }
        ...
        window.openEmployeeDrawer = function(id) {
            ...
            fetch('/hr/employees/' + id + '/json', {
                ...
                .then(function(data) {
                    ...
                    document.getElementById('drawerPosition').innerText = (e.jobPosition || '-') + (e.department ?
                        ' · ' + e.department : '');
                    ...
                    setDrawerText('empDrGender', e.gender === 'Male' ? 'Laki-laki' : (e.gender === 'Female' ?
                        'Perempuan' : e.gender));
                    ...
                    renderEntityNotes('empDrUnifiedNotes', e.notes, e.hrNotes);
                    renderEntityTimeline('empDrTimeline', data.auditLogs);
                    var metaEl = document.getElementById('drawerIdMeta');
                ...
                .catch(function() {
                    renderEntityTimeline('osDrTimeline', []);
                    document.getElementById('drawerCandidateName').innerText = 'Gagal memuat data.';
                });
        };
        ...
        function empOpenEdit(emp) {
            ...
            if (bannerMt) bannerMt.innerText = (emp.jobPosition || emp.jobPositionLocation || '-') + (emp.department ?
                ' · ' + emp.department : '') + (emp.employeeId ? ' · ' + emp.employeeId : '');
            ...
            function setDate(id, val) {
                ...
                if (m) {
                    el.value = m[3] + '-' + m[2].padStart(2, '0') + '-' + m[1].padStart(2, '0');
                    return;
                }
                // Fallback
                var d = new Date(s);
            ...
            // Seksi 6: Catatan
            setVal('efNotes', emp.notes || emp.hrNotes);
        ...
        function persistHrNotes() {
            ...
            fetch('/hr/recruitment/' + activeCandidateId + '/save-notes', {
                ...
                .catch(function() {
                    if (stateEl) {
                        stateEl.innerText = 'Gagal menyimpan';
                        stateEl.className = 'notes-save-state';
                    }
                });
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (notesEl) {
                notesEl.addEventListener('input', function() {
                    ...
                    if (stateEl) {
                        stateEl.innerText = 'Menyimpan...';
                        stateEl.className = 'notes-save-state saving';
                    }
            ...
            if (printBtn) printBtn.addEventListener('click', function() {
                if (activeCandidateId) window.open('/hr/export/candidate-pdf/' + activeCandidateId,
                    '_blank');
            });
            ...
            if (btnBlacklist) {
                btnBlacklist.addEventListener('click', function() {
                    ...
                    formBl.querySelectorAll('textarea').forEach(function(el) {
                        el.value = '';
                    });
