...
// ============================================================
// FIX: Fix existing Audit_Log sheet headers and data
// Run: fixAuditLogSheet() from Apps Script editor
// ============================================================
function fixAuditLogSheet() {
  var lock = LockService.getScriptLock();
  lock.waitLock(60000);
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (!sheet) {
      return "Audit_Log sheet not found. Nothing to fix.";
    }
    ...
    // Check if headers already match AUDIT_LOG_HEADERS
    var headersMatch = currentHeaders.length === AUDIT_LOG_HEADERS.length;
    if (headersMatch) {
      for (var h = 0; h < AUDIT_LOG_HEADERS.length; h++) {
        if (String(currentHeaders[h] || "") !== AUDIT_LOG_HEADERS[h]) {
          headersMatch = false;
          break;
        }
      }
    ...
    // Also check if data column A contains IDs (meaning data is in old format)
    var dataHasOldFormat = false;
    if (existingData.length > 0) {
      var firstColA = String(existingData[0][0] || "");
      if (
        firstColA.indexOf("REC-") === 0 ||
        firstColA.indexOf("EMP-") === 0 ||
        firstColA.indexOf("OS-") === 0
      ) {
        dataHasOldFormat = true;
      }
    }

    // If headers are wrong OR data is in old column order, fix it
    if (headersMatch && !dataHasOldFormat) {
      return "Audit_Log sheet is already correct. No fix needed.";
    }

    var fixedRows = [];

    if (dataHasOldFormat) {
      // Data was written in OLD format: [RecruitmentID, Action, Field, User, OldValue, NewValue, Timestamp]
      // Migrate to new format: [RecruitmentID, Action, Field, OldValue, NewValue, User, Timestamp]
      for (var i = 0; i < existingData.length; i++) {
        var row = existingData[i];
        var recId = String(row[0] || "");
        var action = String(row[1] || "");
        var field = String(row[2] || "");
        var col3 = String(row[3] || "");
        var col4 = String(row[4] || "");
        var col5 = String(row[5] || "");
        var col6 = row[6] || "";

        // Old 7-col format: [RecruitID, Action, Field, User, OldValue, NewValue, Timestamp]
        var oldValue = col4;
        var newValue = col5;
        var user = col3;
        var timestamp = col6;
      ...
      // Data may already be in correct order — just keep it
      for (var j = 0; j < existingData.length; j++) {
        var r = existingData[j];
        // Format any Date objects in timestamp column (index 6)
        if (r.length > 6 && r[6] instanceof Date) {
          var newRow = r.slice();
          newRow[6] = Utilities.formatDate(
            newRow[6],
            "GMT+7",
            "yyyy-MM-dd HH:mm:ss",
          );
    ...
    if (lastRow > 1) {
      sheet
        .getRange(
          ...
          1,
          lastRow - 1,
          Math.max(lastCol, AUDIT_LOG_HEADERS.length),
        )
        .clearContent();
    }
    ...
    if (fixedRows.length > 0) {
      sheet
        .getRange(2, 1, fixedRows.length, AUDIT_LOG_HEADERS.length)
        .setValues(fixedRows);
    }
    ...
    return (
      "Audit_Log fixed! Headers: " +
      AUDIT_LOG_HEADERS.join(", ") +
      ". " +
      fixedRows.length +
      " data rows rewritten."
    );
  } catch (e) {
    Logger.log("ERROR fixAuditLogSheet: " + e.toString());
    return "Error: " + e.toString();
  } finally {
    lock.releaseLock();
  }
