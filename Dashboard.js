    ...
    .withFailureHandler(function (error) {
      ...
      if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="8" class="text-center text-danger py-4">' +
          '<i class="bi bi-exclamation-triangle-fill"></i> Gagal memuat data: ' + error.message + '</td></tr>';
      }
    })
    .getRecruitmentList();
}
...
function populateFilters() {
  ...
  if (filterPosition) {
    ...
    positions.forEach(function (p) {
      filterPosition.innerHTML += '<option value="' + p + '">' + p + '</option>';
    });
  }
  ...
  if (filterEducation) {
    ...
    educations.forEach(function (e) {
      filterEducation.innerHTML += '<option value="' + e + '">' + e + '</option>';
    });
  }
  ...
  if (filterExperience) {
    ...
    experiences.forEach(function (e) {
      filterExperience.innerHTML += '<option value="' + e + '">' + e + '</option>';
    });
  }
}
...
function renderTable() {
  ...
  filteredCandidates.forEach(function (c) {
    ...
    html += '<tr>' +
      ...
      '<td>' + escapeHtml(c.positionApplied || '-') + '</td>' +
      '<td>' + escapeHtml(c.phone || '-') + '</td>' +
      '<td>' + escapeHtml(c.education || '-') + '</td>' +
      '<td>' + escapeHtml(c.workExperience || '-') + '</td>' +
      '<td>' + renderStatusDropdown(c) + '</td>' +
      '<td>' + renderActionButtons(c, phoneClean) + '</td>' +
      '</tr>';
  });
...
function renderStatusDropdown(c) {
  var statusClass = getStatusClass(c.status);
  var html = '<div class="status-dropdown">' +
    '<button class="status-dropdown-btn status-badge ' + statusClass + '" ' +
    'data-recruitment-id="' + escapeHtml(c.recruitmentId) + '">' +
    getStatusLabel(c.status) + ' <i class="bi bi-chevron-down"></i></button>' +
    '<div class="status-dropdown-menu">';

  STATUS_OPTIONS.forEach(function (opt) {
    html += '<div class="status-dropdown-item" ' +
      'data-recruitment-id="' + escapeHtml(c.recruitmentId) + '" ' +
      'data-new-status="' + opt.value + '">' +
  ...
  google.script.run
    .withSuccessHandler(function (response) {
      if (response && response.success) {
        ...
        // Revert on failure
        alert('Gagal mengubah status: ' + (response && response.message ? response.message : 'Unknown error'));
...
function renderActionButtons(c, phoneClean) {
  var html = '<div class="action-btns">' +
    ...
    'data-recruitment-id="' + escapeHtml(c.recruitmentId) + '" ' +
    'data-candidate-name="' + escapeHtml(c.fullName) + '">' +
...
function openCvModal(recruitmentId) {
  ...
  document.getElementById('cvPosition').textContent = candidate.positionApplied || '-';
  document.getElementById('cvPhone').textContent = candidate.phone || '-';
  document.getElementById('cvEmail').textContent = candidate.email || '-';
  document.getElementById('cvEducation').textContent = candidate.education || '-';
  document.getElementById('cvExperience').textContent = candidate.workExperience || '-';
  document.getElementById('cvSalary').textContent = formatSalary(candidate.expectedSalary);
  document.getElementById('cvStatus').textContent = getStatusLabel(candidate.status);
  document.getElementById('cvStatus').className = 'value status-badge ' + getStatusClass(candidate.status);
...
function openDeleteModal(recruitmentId, candidateName) {
  ...
  document.getElementById('btnConfirmDelete').setAttribute('data-recruitment-id', recruitmentId);
  ...
  google.script.run
    .withSuccessHandler(function (response) {
      if (response && response.success) {
        ...
        alert('Gagal menghapus kandidat: ' + (response && response.message ? response.message : 'Unknown error'));
      }
    })
    .withFailureHandler(function (error) {
...
function updateSummaryCards() {
  ...
  var interviewCount = allCandidates.filter(function (c) {
    return c.status === 'HR Interview' || c.status === 'User Interview' || c.status === 'Screening';
  }).length;
  ...
  google.script.run
    .withSuccessHandler(function (logs) {
      if (!logs || logs.length === 0) {
        container.innerHTML = '<div class="text-center py-3 text-muted" style="font-size:12.5px;">Belum ada riwayat aktivitas.</div>';
        return;
      }
      ...
      logs.forEach(function (log) {
        html += '<div class="timeline-item">' +
          ...
          '<span class="timeline-action">' + escapeHtml(log.action) + '</span>' +
          '<span class="timeline-time">' + escapeHtml(log.timestamp) + '</span>' +
          '</div>' +
          '<div class="timeline-body">' +
          '<div class="timeline-user"><i class="bi bi-person-fill"></i> ' + escapeHtml(log.user) + '</div>' +
          (log.oldValue && log.oldValue !== '-' ? '<div class="timeline-change">' + escapeHtml(log.oldValue) + ' \u2192 ' + escapeHtml(log.newValue) + '</div>' : '') +
          '</div></div></div>';
      });
      html += '</div>';
    ...
    .withFailureHandler(function (error) {
      container.innerHTML = '<div class="text-center py-3 text-danger" style="font-size:12.5px;">Gagal memuat riwayat.</div>';
    })
