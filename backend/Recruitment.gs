...
function isNikExists_(nik) {
  ...
  var sheetsToCheck = [
    ...
    { name: HOLD_SHEET_NAME, label: "kandidat_hold", status: "Hold" },
    {
      ...
      label: "kandidat_accepted",
      status: "Accepted",
    },
    {
      name: BLACKLIST_SHEET_NAME,
      label: "kandidat_blacklist",
      status: "Blacklist",
    },
...
function acceptCandidateToEmployee(recruitmentId, hrNotes) {
  ...
  try {
    ...
    if (!existingEmpId) {
      ...
      newRow[EMPLOYEE_COL["NIK - NPWP 16 digit"] - 1] =
        found.values[found.colIndex["NIK"]];
      newRow[EMPLOYEE_COL["NPWP"] - 1] = "";
      newRow[EMPLOYEE_COL["Birth Place"] - 1] = "";
      ...
      newRow[EMPLOYEE_COL["Blood Type"] - 1] = "";
      newRow[EMPLOYEE_COL["PTKP Status"] - 1] = "";
      ...
      newRow[EMPLOYEE_COL["Bank Name"] - 1] = "";
      newRow[EMPLOYEE_COL["Bank Account"] - 1] = "";
      newRow[EMPLOYEE_COL["Bank Account Holder"] - 1] =
        found.values[found.colIndex["Full Name"]] || "";
      newRow[EMPLOYEE_COL["BPJS Ketenagakerjaan"] - 1] = "";
      newRow[EMPLOYEE_COL["BPJS Kesehatan"] - 1] = "";
      ...
      newRow[EMPLOYEE_COL["Type of Rotation"] - 1] = "";
      newRow[EMPLOYEE_COL["Tanggal Mutasi/Demosi/Promosi"] - 1] = "";
      newRow[EMPLOYEE_COL["Nomor SK"] - 1] = "";
      newRow[EMPLOYEE_COL["Resign Date"] - 1] = "";
      ...
      for (var r = 1; r < values.length; r++) {
        if (String(values[r][idCol]) === String(recruitmentId)) {
          ...
          return {
            ...
            currentEmploymentStatus: String(
              cell("Current Employment Status") || "",
            ),
            availableToJoin: String(cell("Available to Join") || ""),
            expectedSalary:
              cell("Expected Salary") === ""
                ? 0
                : Number(cell("Expected Salary")),
...
function buildStatusRow_(found, targetHeaders, nowStr, user) {
  ...
  Object.keys(srcIdx).forEach(function (colName) {
    if (tgtIdx[colName] !== undefined) {
      row[tgtIdx[colName]] =
        src[srcIdx[colName]] !== undefined ? src[srcIdx[colName]] : "";
    }
  });

  if (tgtIdx["Processed Date"] !== undefined)
    row[tgtIdx["Processed Date"]] = nowStr;
...
function getStatusSheetList_(sheetName) {
  ...
  for (var r = 1; r < values.length; r++) {
    ...
    result.push({
      ...
      offeringCreated: fmtDate_(
        cell(row, "Offering Created"),
        "dd/MM/yyyy HH:mm",
      ),
      offeringUpdated: fmtDate_(
        cell(row, "Offering Updated"),
        "dd/MM/yyyy HH:mm",
      ),
      ...
      offeringResponseDate: fmtDate_(
        cell(row, "Offering Response Date"),
        "dd/MM/yyyy HH:mm",
      ),
      offeringResponseBy: String(cell(row, "Offering Response By") || ""),
      onboardingStatus: String(cell(row, "Onboarding Status") || ""),
      onboardingDate: fmtDate_(
        cell(row, "Onboarding Date"),
        "dd/MM/yyyy HH:mm",
      ),
  ...
  result.sort(function (a, b) {
    ...
    return (
      toTs(b.processedDate || b.createdDate) -
      toTs(a.processedDate || a.createdDate)
    );
  });
...
function getHoldList() {
  return getStatusSheetList_(HOLD_SHEET_NAME);
}
function getAcceptedList() {
  return getStatusSheetList_(ACCEPTED_SHEET_NAME);
}
function getBlacklistList() {
  return getStatusSheetList_(BLACKLIST_SHEET_NAME);
}

// ---- Ubah status antar sheet status (hold/accepted/blacklist) ----
function moveStatusCandidate(
  recruitmentId,
  fromStatus,
  toStatus,
  reason,
  ...
  var sheetNameMap = {
    Hold: HOLD_SHEET_NAME,
    Accepted: ACCEPTED_SHEET_NAME,
    Blacklist: BLACKLIST_SHEET_NAME,
  };
  var headersMap = {
    Hold: HOLD_HEADERS,
  ...
  var sheetFnMap = {
    Hold: getOrCreateHoldSheet_,
    Accepted: getOrCreateAcceptedSheet_,
    Blacklist: getOrCreateBlacklistSheet_,
  };
  ...
  try {
    ...
    if (!srcSheet || srcSheet.getLastRow() < 2)
      return {
        success: false,
        message: "Sheet " + fromStatus + " tidak ditemukan.",
      };
    ...
    if (!found)
      return {
        success: false,
        message:
          "Recruitment ID tidak ditemukan di " +
          fromStatus +
          ": " +
          recruitmentId,
      };
    ...
    if (toStatus === "Accepted") {
      ...
      if (!existingEmpId) {
        ...
        if (empIdCol)
          dstSheet.getRange(accLastRow, empIdCol).setValue(employeeId);

        var empSheet = getOrCreateEmployeeSheet_();
        var empRow = new Array(EMPLOYEE_HEADERS.length).fill("");
        empRow[EMPLOYEE_COL["Employee ID"] - 1] = employeeId;
        empRow[EMPLOYEE_COL["Full Name"] - 1] =
          found.values[found.colIndex["Full Name"]];
        empRow[EMPLOYEE_COL["Position"] - 1] =
          found.values[found.colIndex["Position Applied"]];
        empRow[EMPLOYEE_COL["Email"] - 1] =
          found.values[found.colIndex["Email"]];
        empRow[EMPLOYEE_COL["Phone"] - 1] =
          found.values[found.colIndex["Phone"]];
        empRow[EMPLOYEE_COL["Join Date"] - 1] = Utilities.formatDate(
          now,
          "GMT+7",
          "yyyy-MM-dd",
        );
        ...
        empRow[EMPLOYEE_COL["Birth Date"] - 1] =
          found.values[found.colIndex["Birth Date"]];
        empRow[EMPLOYEE_COL["Gender"] - 1] =
          found.values[found.colIndex["Gender"]];
        empRow[EMPLOYEE_COL["Marital Status"] - 1] =
          found.values[found.colIndex["Marital Status"]];
        empRow[EMPLOYEE_COL["Citizen ID Address"] - 1] =
          found.values[found.colIndex["Address"]] || "";
        empRow[EMPLOYEE_COL["Residential Address"] - 1] =
          found.values[found.colIndex["Address"]] || "";
...
function saveOfferingStatus(recruitmentId, generatedBy, offerData) {
  ...
  try {
    ...
    if (!sheet || sheet.getLastRow() < 2)
      return {
        success: false,
        message: "Sheet kandidat_accepted tidak ditemukan.",
      };
    ...
    if (!idCol)
      return {
        success: false,
        message: "Kolom Recruitment ID tidak ditemukan.",
      };
    ...
    if (offeringCreatedCol) {
      existingCreated = sheet
        .getRange(targetRow, offeringCreatedCol)
        .getValue();
    }
    ...
    if (!existingCreated || String(existingCreated).trim() === "") {
      ...
      if (offeringCreatedCol)
        sheet.getRange(targetRow, offeringCreatedCol).setValue(nowStr);
      if (offeringCreatedByCol)
        sheet.getRange(targetRow, offeringCreatedByCol).setValue(user);
      if (respCol) {
      ...
      if (offeringUpdatedCol)
        sheet.getRange(targetRow, offeringUpdatedCol).setValue(nowStr);
      if (offeringUpdatedByCol)
        sheet.getRange(targetRow, offeringUpdatedByCol).setValue(user);
      writeAuditLog_(
        recruitmentId,
        "Offering Letter Updated",
        "Offering Updated",
        "-",
        nowStr + " by " + user,
      );
...
function saveOfferingResponse(recruitmentId, response, notes, updatedBy) {
  ...
  try {
    ...
    var user =
      updatedBy || Session.getActiveUser().getEmail() || "HR Dashboard";

    if (respCol)
      sheet.getRange(targetRow, respCol).setValue(response || "Menunggu");
    if (respNotesCol)
      sheet.getRange(targetRow, respNotesCol).setValue(notes || "");
  ...
  try {
    ...
    if (!sheet || sheet.getLastRow() < 2) {
      return {
        success: false,
        message:
          "Data tidak ditemukan. Pastikan Anda sudah diterima sebagai karyawan.",
      };
    }
    ...
    return {
      success: false,
      message:
        "Data dengan NIK tersebut tidak ditemukan. Pastikan Anda sudah diterima sebagai karyawan.",
    };
  } catch (err) {
    return { success: false, message: err.message };
  }
