...
@section('page-subtitle', $isManager ? 'Pengajuan dan monitoring kebutuhan tenaga kerja' : 'Manajemen permintaan kebutuhan manpower dari departemen')

@section('styles')
<style>
  .mpr-card-header {
    background: linear-gradient(135deg, #005BAC 0%, #00437e 100%);
    color: #ffffff;
...
<div class="container-fluid px-0">
      ...
      <div class="col-12 col-xl-7">
        <div class="card border-0 shadow-sm rounded-3">
          <div class="mpr-card-header d-flex justify-content-between align-items-center">
            <div>
              <h5 class="mb-1 fw-bold text-white"><i class="bi bi-file-earmark-plus-fill me-2"></i> Formulir Pengajuan Manpower Request</h5>
              <small class="text-white-50">Isi data kebutuhan tenaga kerja untuk diproses oleh Tim HR</small>
            </div>
            ...
            <form id="formManagerMpr" action="{{ route('hr.mpr.store') }}" method="POST">
              ...
              <div class="bg-light p-3 rounded-3 mb-4 border">
                <div class="d-flex align-items-center justify-content-between mb-2">
                  ...
                  <span class="badge bg-secondary-subtle text-secondary border"><i class="bi bi-lock-fill me-1"></i> Terkunci Permanen</span>
                </div>
                <div class="row g-3">
                  <div class="col-md-4">
                    ...
                    <div class="input-group input-group-sm">
                      ...
                      <input type="text" class="form-control bg-white border-start-0 text-dark fw-semibold" value="{{ $user['fullName'] ?? $user['name'] ?? 'Manager' }}" readonly disabled style="cursor: not-allowed;">
                    </div>
                  </div>
                  <div class="col-md-4">
                    ...
                    <div class="input-group input-group-sm">
                      ...
                      <input type="text" class="form-control bg-white border-start-0 text-dark fw-semibold" value="{{ $user['email'] ?? '-' }}" readonly disabled style="cursor: not-allowed;">
                    </div>
                  </div>
                  <div class="col-md-4">
                    ...
                    <div class="input-group input-group-sm">
                      ...
                      <input type="text" class="form-control bg-white border-start-0 text-dark fw-semibold" value="{{ !empty($user['branch']) ? $user['branch'] : '-' }}" readonly disabled style="cursor: not-allowed;" title="Branch diambil otomatis dari profil akun Anda">
                    </div>
                  </div>
                </div>
                ...
                <div class="row g-3 mt-1">
                  <div class="col-md-6">
                    ...
                    @elseif(count($allowedEntities) === 1)
                      ...
                      @php $singleEntityCode = array_key_first($allowedEntities); $singleEntityLabel = $allowedEntities[$singleEntityCode]; @endphp
                      <div class="input-group input-group-sm">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-building-fill"></i></span>
                        <input type="text" class="form-control bg-white border-start-0 text-dark fw-semibold" value="{{ $singleEntityCode }} — {{ $singleEntityLabel }}" readonly disabled style="cursor: not-allowed;">
                      </div>
              ...
              <div class="row g-3 mb-3">
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Posisi / Jabatan yang Diminta <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Level Jabatan <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Lokasi Kerja Penempatan <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Jumlah Kebutuhan (Orang) <span class="text-danger">*</span></label>
                  <input type="number" name="quantity" class="form-control" value="1" min="1" max="100" required>
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Target Tanggal Bergabung (Join Date) <span class="text-danger">*</span></label>
                  <input type="date" name="expected_join_date" class="form-control" min="{{ date('Y-m-d') }}" required>
                </div>
              </div>

              <!-- SECTION: ALASAN PERMINTAAN -->
              <h6 class="fw-bold text-primary mb-3 mt-4"><i class="bi bi-question-circle-fill me-2"></i>Alasan & Kualifikasi</h6>
              <div class="row g-3 mb-3">
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Alasan Permintaan <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Nama Karyawan yang Digantikan (Opsional)</label>
      ...
      <div class="col-12 col-xl-5">
        <div class="card border-0 shadow-sm rounded-3 h-100">
          <div class="card-header bg-white py-3 border-bottom">
            <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-clock-history me-2 text-primary"></i> Riwayat Pengajuan MPR Saya</h6>
          </div>
          <div class="card-body p-0">
            <div class="table-responsive">
              <table class="table table-mpr table-hover mb-0">
                <thead>
                  <tr>
                    <th>No. MPR</th>
                    <th>Posisi & Dept</th>
                ...
                <tbody id="managerMprTableBody">
                  ...
                  <tr>
                    <td>
                      ...
                      <small class="text-muted">{{ $mpr->requestDate ? date('d/m/Y', strtotime($mpr->requestDate)) : '-' }}</small>
                    </td>
                    <td>
                      <span class="fw-semibold text-dark">{{ $mpr->position }}</span><br>
                      <small class="text-muted">{{ $mpr->department }}</small><br>
                      <small class="badge bg-light text-secondary border" style="font-size:10px;">{{ $mpr->entity ?: $mpr->company }}</small>
                    </td>
                    <td>
                      <span class="badge bg-light text-dark border">{{ $mpr->quantity }} Org</span>
                    </td>
                    <td>
                      <div class="btn-group btn-group-sm">
                        <button type="button" class="btn btn-outline-primary btn-mpr-detail" data-id="{{ $mpr->mprNumber }}" title="Lihat Detail">
                          <i class="bi bi-eye"></i>
                        </button>
                        <a href="{{ route('hr.mpr.pdf', $mpr->mprNumber) }}" target="_blank" class="btn btn-outline-danger" title="Unduh PDF">
  ...
  @else
      ...
      <div class="col-12 col-sm-6 col-xl-3">
        <div class="stat-card-mpr">
          ...
          <div>
            ...
            <div class="fs-4 fw-bold text-dark">{{ $stats['total_quantity'] ?? 0 }} <span class="fs-6 fw-normal text-muted">Orang</span></div>
          </div>
        </div>
      </div>
      <div class="col-12 col-sm-6 col-xl-3">
    ...
    <div class="card border-0 shadow-sm rounded-3 mb-4">
      <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center gap-2 border-bottom">
        ...
        <div class="d-flex gap-2">
          @can('create_mpr')
          <button type="button" class="btn btn-primary btn-sm fw-semibold shadow-sm" data-bs-toggle="modal" data-bs-target="#modalCreateMpr">
        ...
        <form action="{{ route('hr.mpr.index') }}" method="GET" class="row g-2 align-items-center">
          <div class="col-12 col-md-4">
            <div class="input-group input-group-sm">
              ...
              <input type="text" name="search" class="form-control border-start-0" placeholder="Cari No MPR, pemohon, posisi, divisi..." value="{{ $search }}">
            </div>
          </div>
          ...
          <div class="col-12 col-md-3 d-flex gap-2">
            ...
            <a href="{{ route('hr.mpr.index') }}" class="btn btn-sm btn-outline-secondary" title="Reset Filter"><i class="bi bi-arrow-counterclockwise"></i></a>
          </div>
        </form>
      </div>
            ...
            <tbody>
              ...
              <tr>
                <td>
                  ...
                  <small class="text-muted"><i class="bi bi-calendar3 me-1"></i>{{ $mpr->requestDate ? date('d M Y', strtotime($mpr->requestDate)) : '-' }}</small>
                </td>
                <td>
                ...
                <td>
                  ...
                  @if(!empty($mpr->branch))
                    <div><small class="text-muted" style="font-size:10px;"><i class="bi bi-geo-alt me-1"></i>{{ $mpr->branch }}</small></div>
                  @endif
                </td>
                <td>
                  <div class="fw-semibold text-dark">{{ $mpr->position }}</div>
                  <small class="badge bg-light text-secondary border">{{ $mpr->jobLevel ?? '-' }}</small>
                  <small class="badge bg-light text-secondary border">{{ $mpr->employmentType ?? '-' }}</small>
                </td>
                ...
                <td>
                  <span class="text-dark">{{ $mpr->expectedJoinDate ? date('d M Y', strtotime($mpr->expectedJoinDate)) : '-' }}</span>
                </td>
                <td>
                  <span class="badge-mpr-submitted">{{ strtoupper($mpr->status ?? 'SUBMITTED') }}</span>
                </td>
                <td class="text-center">
                  <div class="btn-group btn-group-sm">
                    <button type="button" class="btn btn-outline-primary btn-mpr-detail" data-id="{{ $mpr->mprNumber }}" title="Lihat Detail">
                    ...
                    @can('export_mpr')
                    <a href="{{ route('hr.mpr.pdf', $mpr->mprNumber) }}" target="_blank" class="btn btn-outline-danger" title="Unduh PDF Resmi">
      ...
      <div class="card-footer bg-white py-3 d-flex justify-content-between align-items-center border-top">
        <div class="small text-muted">
          Menampilkan halaman <strong>{{ $currentPage }}</strong> dari <strong>{{ $lastPage }}</strong>
        </div>
        <ul class="pagination pagination-sm mb-0">
          <li class="page-item {{ $currentPage <= 1 ? 'disabled' : '' }}">
            <a class="page-link" href="{{ request()->fullUrlWithQuery(['page' => $currentPage - 1]) }}">Sebelumnya</a>
          </li>
          ...
          <li class="page-item {{ $currentPage >= $lastPage ? 'disabled' : '' }}">
            <a class="page-link" href="{{ request()->fullUrlWithQuery(['page' => $currentPage + 1]) }}">Berikutnya</a>
          </li>
        </ul>
      </div>
      @endif
    ...
    @can('create_mpr')
    <div class="modal fade" id="modalCreateMpr" tabindex="-1" aria-labelledby="modalCreateMprLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content border-0 shadow">
          <div class="modal-header bg-primary text-white">
            <h5 class="modal-title fw-bold" id="modalCreateMprLabel"><i class="bi bi-plus-circle me-2"></i> Buat Pengajuan Manpower Request (MPR)</h5>
          ...
          <div class="modal-body p-4">
            <form id="formHrCreateMpr" action="{{ route('hr.mpr.store') }}" method="POST">
              ...
              <div class="row g-3 mb-4 bg-light p-3 rounded-3 border">
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Nama Manager Pemohon <span class="text-danger">*</span></label>
                  <input type="text" name="manager_name" class="form-control form-control-sm" value="{{ $user['fullName'] ?? 'HR Manager' }}" required>
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Email Manager Pemohon <span class="text-danger">*</span></label>
                  <input type="email" name="manager_email" class="form-control form-control-sm" value="{{ $user['email'] ?? '' }}" required>
                </div>
                <div class="col-md-12">
                  <label class="form-label fw-semibold small">Entitas / Perusahaan <span class="text-danger">*</span></label>
                  <select name="entity" class="form-select form-select-sm" required>
                    <option value="">-- Pilih Entitas --</option>
                  ...
                  <div class="form-text text-muted" style="font-size:11px;">Pilih entitas (company) yang akan tercantum di dokumen MPR ini.</div>
                </div>
              </div>
              ...
              <div class="row g-3 mb-4">
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Posisi / Jabatan yang Diminta <span class="text-danger">*</span></label>
                  <input type="text" name="position" class="form-control form-control-sm" placeholder="Contoh: Digital Marketing Lead" required>
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Departemen <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Lokasi Penempatan <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Target Tanggal Bergabung <span class="text-danger">*</span></label>
                  <input type="date" name="expected_join_date" class="form-control form-control-sm" required>
                </div>
              </div>
              ...
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  <label class="form-label fw-semibold small">Nama Karyawan yang Digantikan (Opsional)</label>
                  <input type="text" name="replacement_for" class="form-control form-control-sm" placeholder="Diisi jika penggantian">
                </div>
                <div class="col-12">
                  <label class="form-label fw-semibold small">Kualifikasi & Persyaratan Khusus</label>
                  <textarea name="requirements" class="form-control form-control-sm" rows="3" placeholder="Pendidikan, pengalaman kerja, keahlian teknis..."></textarea>
                </div>
                <div class="col-12">
                  <label class="form-label fw-semibold small">Uraian Tugas Utama</label>
                  <textarea name="job_description" class="form-control form-control-sm" rows="3" placeholder="Ringkasan tanggung jawab posisi..."></textarea>
                </div>
                <div class="col-12">
                  <label class="form-label fw-semibold small">Catatan Tambahan (Opsional)</label>
              ...
              <div class="modal-footer px-0 pb-0 mt-4 border-top">
                ...
                <button type="submit" id="btnSubmitHrCreateMpr" class="btn btn-primary btn-sm fw-semibold">
                  <span class="spinner-border spinner-border-sm me-1 d-none" id="spinnerSubmitHr"></span>
                  <i class="bi bi-check-circle-fill me-1" id="iconSubmitHr"></i> Simpan MPR & Generate PDF
                </button>
              </div>
            </form>
          </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content border-0 shadow">
        <div class="modal-header bg-dark text-white">
          <div>
            <h5 class="modal-title fw-bold" id="modalMprDetailLabel"><i class="bi bi-file-earmark-text me-2"></i> Detail Manpower Request</h5>
            <small class="text-white-50" id="detailMprNumber">-</small>
          </div>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
          ...
          <div id="detailContent" class="d-none">
            ...
            <div class="row g-3 mb-3">
              ...
              <div class="col-md-6">
                ...
                <div class="detail-val"><span class="badge bg-primary fs-6" id="detQuantity">-</span> Orang</div>
              </div>
              <div class="col-md-6">
        ...
        <div class="modal-footer bg-light border-top">
          ...
          <a id="btnModalDownloadPdf" href="#" target="_blank" class="btn btn-danger btn-sm fw-semibold">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setupMprForm('formManagerMpr', 'btnSubmitMprManager', 'spinnerSubmitManager', 'iconSubmitManager', null);
  setupMprForm('formHrCreateMpr', 'btnSubmitHrCreateMpr', 'spinnerSubmitHr', 'iconSubmitHr', 'modalCreateMpr');
  ...
  detailButtons.forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      .then(async response => {
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Gagal memuat detail MPR.');
        return data;
      })
      .then(data => {
        const m = data.mpr;
        document.getElementById('detManagerName').innerText = m.requestor_name || m.manager_name || '-';
        document.getElementById('detManagerEmail').innerText = m.requestor_email || m.manager_email || '-';
        document.getElementById('detCompany').innerText = m.entity || m.company || '-';
        const detBranch = document.getElementById('detBranch');
        if (detBranch) detBranch.innerText = m.branch ? `Branch: ${m.branch}` : '';
        document.getElementById('detCreatedBy').innerText = `Diajukan: ${m.created_at || m.request_date || '-'}`;
        document.getElementById('detPosition').innerText = m.position || '-';
        document.getElementById('detDeptDiv').innerText = `${m.department || '-'} / ${m.division || '-'}`;
        document.getElementById('detJobLevel').innerText = m.job_level || '-';
        document.getElementById('detEmpType').innerText = m.employment_type || '-';
        document.getElementById('detLocation').innerText = m.work_location || '-';
        document.getElementById('detQuantity').innerText = m.quantity || '1';
        document.getElementById('detJoinDate').innerText = m.expected_join_date || '-';
        ...
        if (m.replacement_for) {
          document.getElementById('detReplacementFor').innerText = m.replacement_for;
      ...
      .catch(error => {
        loadingEl.innerHTML = `<div class="text-danger py-4"><i class="bi bi-exclamation-triangle-fill fs-2 d-block mb-2"></i>${error.message}</div>`;
      });
    });
  });
