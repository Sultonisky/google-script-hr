  ...
  <div class="container">
    <div class="row align-items-center">
      <div class="col-lg-7 col-md-8">
        ...
        <p class="hero-subtitle">Perbarui kontak aktif, alamat domisili, atau kualifikasi pendidikan untuk pendaftaran nomor {{ $candidate->recruitmentId }}.</p>
      </div>
  ...
  <div class="container" style="max-width:850px;">
    <div class="card border-0 shadow-sm rounded-4" style="border:1px solid #e2e8f0!important;">
      ...
      <div class="card-body p-4">
        <div class="alert alert-info border-0 mb-4 p-3 rounded-3" style="background:#f0f7ff;color:#005BAC;font-size:13px">
          <i class="bi bi-info-circle-fill me-1"></i> Anda sedang memperbarui data untuk pendaftaran: <strong>{{ $candidate->recruitmentId }}</strong> ({{ $candidate->fullName }} &bull; {{ $candidate->positionApplied }}).
        </div>

        @if(session('success'))
          <div class="alert alert-success alert-dismissible fade show p-3 mb-4 rounded-3 d-flex align-items-center gap-2" role="alert">
        ...
        <form action="{{ route('public.career.self-update.store', ['id' => $candidate->recruitmentId]) }}" method="POST">
          @csrf
          ...
          <div class="row g-3 mb-4" style="font-size:13px">
            ...
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" name="no_telp" value="{{ old('no_telp', $candidate->phone) }}" required>
            </div>
            <div class="col-md-12">
            ...
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" name="pendidikan" value="{{ old('pendidikan', $candidate->education) }}">
            </div>
            <div class="col-md-6">
              <label class="form-label fw-semibold">Pengalaman Kerja</label>
              <input type="text" class="form-control" name="pengalaman_kerja" value="{{ old('pengalaman_kerja', $candidate->workExperience) }}">
            </div>
            <div class="col-md-12">
              <label class="form-label fw-semibold">Tautan / Link CV Terbaru</label>
              <input type="url" class="form-control" name="cv_link" value="{{ old('cv_link', $candidate->cvLink) }}" placeholder="https://drive.google.com/...">
            </div>
          </div>

          <div class="d-flex justify-content-end gap-2 border-top pt-3">
            <a href="{{ route('public.career.check-status', ['q' => $candidate->recruitmentId]) }}" class="btn btn-light border px-4" style="border-radius:10px;font-size:13.5px">Batal</a>
