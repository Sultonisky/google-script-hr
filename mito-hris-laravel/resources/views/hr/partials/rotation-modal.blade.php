            ...
            <form action="" method="POST" id="formRotation">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="position-relative">
                            <input type="text" class="form-control" id="rotEmpSearch"
                                ...
                                style="font-size:13px;padding-right:36px" oninput="handleRotEmpSearch(this.value)" />
                    ...
                    <div id="rotEmpPreview" style="display:none">
                        ...
                        <div id="rotFormSection">
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold" style="font-size:13px">Tipe Rotasi <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select form-select-sm" name="rotation_type" id="rotType"
                                        required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control form-control-sm" name="effective_date"
                                        id="rotEffectiveDate" value="{{ date('Y-m-d') }}" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold" style="font-size:13px">Jabatan Baru <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm"
                                        name="new_job_position" id="rotNewPosition"
                                        placeholder="Contoh: HR Manager (Jakarta)" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold" style="font-size:13px">Departemen Baru <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-sm" name="new_department"
                                        id="rotNewDepartment" placeholder="Contoh: Operations / Sales" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold" style="font-size:13px">Cabang Baru</label>
                                    <select class="form-select form-select-sm" name="new_branch_name"
                                        id="rotNewBranch">
                                ...
                                <div class="col-md-12">
                                    <label class="form-label fw-semibold" style="font-size:13px">Alasan / Catatan
                                        Rotasi</label>
                                    <textarea class="form-control form-control-sm" name="notes" id="rotReason" rows="2"
                                        placeholder="Contoh: Penyesuaian struktur organisasi..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-sm text-white fw-semibold" style="background:#eb1c24"
                        id="btnConfirmRotation" disabled>
...
<script>
    ...
    if (typeof window.__allEmployees === 'undefined' || !Array.isArray(window.__allEmployees) || !window.__allEmployees
        .length) {
    ...
    function handleRotEmpSearch(query) {
        ...
        dropdown.innerHTML = matched.map(function(emp) {
            return '<div class="p-2 border-bottom d-flex align-items-center gap-2 hover-item rot-search-item" style="cursor:pointer;" data-emp-id="' +
                (emp.employeeId || '') + '">' +
        ...
        dropdown.querySelectorAll('.rot-search-item').forEach(function(item) {
            item.addEventListener('click', function() {
                ...
                var found = (window.__allEmployeesForRotation || []).find(function(e) {
            ...
            fetch(form.action, {
                    ...
                    headers: {
                        ...
                        'X-CSRF-TOKEN': (document.querySelector('meta[name="csrf-token"]') || {})
                            .content || (form.querySelector('input[name="_token"]') || {}).value ||
                            ''
                    },
                ...
                .then(function(res) {
                    ...
                    if (!res || !res.success) {
                        _showRotToast('Gagal: ' + (res ? (res.message || 'Error tidak diketahui') :
                ...
                .catch(function(err) {
                    btn.disabled = false;
                    btn.innerHTML = origBtnHtml;
                    _showRotToast('Error: ' + (err ? (err.message || String(err)) :
                        'Network error'), 'error');
                });
        });
    });
