...
<head>
    ...
    <style>
        ...
        * {
        ...
        .eval-table .col-kompeten {
            width: 100px;
            font-weight: bold;
            font-style: italic;
        }

        .eval-table .col-action {
            width: 130px;
        }

        .eval-table .col-indicator {}

        .eval-table .col-check {
            width: 52px;
            text-align: center;
        }

        .eval-table .row-evidence td {
            background: #f0f0f0;
            font-style: italic;
            font-size: 7.5pt;
        }

        .eval-table .row-total td {
            background: #e8f0fb;
            font-weight: bold;
            text-align: right;
        }

        .eval-table .row-total .total-val {
            font-size: 11pt;
            font-weight: 900;
            color: #eb1c24;
        }
...
<body>
    ...
    @php
        ...
        // ── Indicator values (1 = checked, 0/'' = ✗) ─────────────────────
        $indVal = function (string $key) use ($evalData): bool {
            $v = $evalData[$key] ?? ($evalData['ind_' . $key] ?? 0);
            return !empty($v) && $v !== '0' && $v !== false;
        };

        // ── SVG checkbox helpers (avoid font dependency) ────────────────
        $svgChecked = '<svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg"><rect width="14" height="14" rx="2" fill="#eb1c24"/><path d="M3 7 L6 10 L11 3" stroke="white" stroke-width="2" fill="none"/></svg>';
        $svgUnchecked = '<svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg"><rect width="14" height="14" rx="2" fill="white" stroke="#555" stroke-width="1.5"/></svg>';
        $svgCheckedSmall = '<svg width="12" height="12" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg"><rect width="12" height="12" rx="2" fill="#eb1c24"/><path d="M2.5 5.5 L5 8.5 L9.5 3" stroke="white" stroke-width="2" fill="none"/></svg>';
        $svgUncheckedSmall = '<svg width="12" height="12" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg"><rect width="12" height="12" rx="2" fill="white" stroke="#555" stroke-width="1.5"/></svg>';
        $chk = function($v) use ($svgChecked, $svgUnchecked) { return $v ? $svgChecked : $svgUnchecked; };
        $chkSmall = function($v) use ($svgCheckedSmall, $svgUncheckedSmall) { return $v ? $svgCheckedSmall : $svgUncheckedSmall; };

        // Per-competency totals (prefer stored; fall back to counting indicators)
        $intTotal =
            (int) ($evalData['integrityTotal'] ??
                (int) $indVal('integrity_1') +
                    (int) $indVal('integrity_2') +
                    (int) $indVal('integrity_3') +
                    (int) $indVal('integrity_4'));
        $ciTotal =
            (int) ($evalData['ciTotal'] ??
                (int) $indVal('ci_1') + (int) $indVal('ci_2') + (int) $indVal('ci_3') + (int) $indVal('ci_4'));
        $eeTotal = (int) ($evalData['eeTotal'] ?? (int) $indVal('ee_1') + (int) $indVal('ee_2'));
        $twTotal =
            (int) ($evalData['twTotal'] ?? (int) $indVal('tw_1') + (int) $indVal('tw_2') + (int) $indVal('tw_3'));
        ...
        $isPutus =
            str_contains($decision, 'Tidak Lulus') ||
    ...
    <table class="eval-table">
        ...
        <tbody>
            ...
            {{-- ─── INTEGRITY ─────────────────────────────────────── --}}
            @php $rows_integrity = [['integrity_1', 'Menyelesaikan minimal 90% tugas sesuai deadline.'], ['integrity_2', 'Memberikan update progres pekerjaan secara rutin.'], ['integrity_3', 'Menindaklanjuti permasalahan sesuai SLA.'], ['integrity_4', 'Tidak terdapat pelanggaran prosedur, kebijakan, atau komitmen kerja yang telah disepakati.']]; @endphp

            @foreach ($rows_integrity as $i => $row)
                <tr>
                    @if ($i === 0)
                        <td class="col-kompeten" rowspan="{{ count($rows_integrity) + 2 }}"
                            style="font-style:italic;vertical-align:middle">Integrity</td>
                        <td class="col-action" rowspan="{{ count($rows_integrity) }}" style="vertical-align:top">
                            <strong>Take Accountability</strong><br>
                            <span style="font-weight:normal">Bertanggung jawab atas tindakan, keputusan &amp; hasil
                                kerja yang dilakukan.</span>
                        </td>
                    @endif
                    <td>{{ $row[1] }}</td>
                    <td class="col-check">
                        <span
                            class="chk-box {{ $indVal($row[0]) ? 'chk-yes' : 'chk-no' }}">{{ $indVal($row[0]) ? '✓' : '✗' }}</span>
                    </td>
                </tr>
            @endforeach
            ...
            @php $rows_ci = [['ci_1', 'Mengusulkan minimal 1 improvement atau solusi selama masa probation.'], ['ci_2', 'Berpartisipasi dalam minimal 1 project atau kegiatan tim atau perusahaan.'], ['ci_3', 'Mempelajari atau mengimplementasi proses, sistem atau knowledge baru yang dapat mendukung pekerjaan.'], ['ci_4', 'Mengambil tindakan awal terhadap masalah yang ditemukan sebelum dilakukan eskalasi.']]; @endphp

            @foreach ($rows_ci as $i => $row)
                <tr>
                    @if ($i === 0)
                        <td class="col-kompeten" rowspan="{{ count($rows_ci) + 2 }}"
                            style="font-style:italic;vertical-align:middle">Continuous<br>Improvement</td>
                        <td class="col-action" rowspan="{{ count($rows_ci) }}" style="vertical-align:top">
                            <strong>Proactive Contribution</strong><br>
                            <span style="font-weight:normal">Aktif mencari kesempatan belajar &amp; meningkatkan cara
                                kerja.</span>
                        </td>
                    @endif
                    <td>{{ $row[1] }}</td>
            ...
            {{-- ─── E✗ECUTION E✗CELLENCE ──────────────────────────── --}}
            @php $rows_ee = [['ee_1', 'Tingkat kesalahan atau rework tidak melebihi kesepakatan yang telah ditetapkan.'], ['ee_2', 'Hasil pekerjaan dapat digunakan atau diselesaikan tanpa koreksi mayor.']]; @endphp

            @foreach ($rows_ee as $i => $row)
                <tr>
                    @if ($i === 0)
                        <td class="col-kompeten" rowspan="{{ count($rows_ee) + 2 }}"
                            style="font-style:italic;vertical-align:middle">Execution<br>Excellence</td>
            ...
            {{-- ─── TEAMWORK ───────────────────────────────────────── --}}
            @php $rows_tw = [['tw_1', 'Berpartisipasi aktif dalam meeting, diskusi, atau koordinasi yang berkaitan dengan pekerjaan.'], ['tw_2', 'Menindaklanjuti permintaan atau kebutuhan stakeholder internal sesuai SLA.'], ['tw_3', 'Tidak terdapat keluhan mayor terkait koordinasi atau kerja sama selama masa probation.']]; @endphp

            @foreach ($rows_tw as $i => $row)
                <tr>
                    @if ($i === 0)
                        <td class="col-kompeten" rowspan="{{ count($rows_tw) + 2 }}"
                            style="font-style:italic;vertical-align:middle">Teamwork</td>
                        <td class="col-action" rowspan="{{ count($rows_tw) }}" style="vertical-align:top">
                            <strong>Supportive Collaboration</strong><br>
                            <span style="font-weight:normal">Berkolaborasi dan memberikan dukungan untuk mencapai
                                tujuan/target bersama.</span>
                        </td>
                    @endif
                    <td>{{ $row[1] }}</td>
                    <td class="col-check">
            ...
            <tr class="score-cat-row">
                ...
                @foreach ($cats as $cat)
                    <td style="width:12%;padding:2px 3px">
                        <span
                            style="display:inline-block;padding:1px 5px;border:1.5px solid {{ $cat[2] }};border-radius:3px;
              {{ $category === $cat[1] ? 'background:' . $cat[2] . ';color:#fff;' : 'color:' . $cat[2] . ';' }}
              font-weight:700;font-size:8pt">{{ $cat[0] }}</span>
                    </td>
                    <td
                        style="width:13%;font-size:8pt;color:{{ $category === $cat[1] ? $cat[2] : '#555' }};
    ...
    <div class="decision-box">
        ...
        <div style="margin-bottom:4px">
            <span
                class="chk-inline {{ $isLulus && !$isPerp && !$isPutus ? 'checked' : '' }}">{!! $isLulus && !$isPerp && !$isPutus ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
            <strong>Diangkat sebagai Karyawan Tetap</strong>
        </div>
        <div style="margin-bottom:4px">
            <span class="chk-inline {{ $isPutus ? 'checked' : '' }}">{!! $isPutus ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
    ...
    <table class="approval-table">
        ...
        <tr>
            <td>
                <span
                    class="chk-inline {{ $apDept === 'Setuju' ? 'checked' : '' }}">{!! $apDept === 'Setuju' ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
                Setuju &nbsp;&nbsp;
                <span
                    class="chk-inline {{ $apDept === 'Tidak' ? 'checked' : '' }}">{!! $apDept === 'Tidak' ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
                Tidak
            </td>
            <td>
                <span
                    class="chk-inline {{ $apHrbp === 'Setuju' ? 'checked' : '' }}">{!! $apHrbp === 'Setuju' ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
                Setuju &nbsp;&nbsp;
                <span
                    class="chk-inline {{ $apHrbp === 'Tidak' ? 'checked' : '' }}">{!! $apHrbp === 'Tidak' ? '<span style="font-family:DejaVu Sans;">✓</span>' : '' !!}</span>
                Tidak
            </td>
        </tr>
        <tr>
    ...
    <div style="font-size:7pt;color:#aaa;text-align:right;margin-top:6px">
        Evaluation Form, pg. 1 &nbsp;|&nbsp; Generated: {{ $todayStr }} &nbsp;|&nbsp; Eval ID:
        {{ $evalData['evalId'] ?? '-' }}
    </div>
