...
function simpanDataOutsource(formObject) {
  try {
    ...
    newRow[OUTSOURCE_COL['Work Experience'] - 1]    = formObject.work_experience || '';
    newRow[OUTSOURCE_COL['Vendor Company'] - 1]     = formObject.vendor_company || '';
    newRow[OUTSOURCE_COL['Contract Number'] - 1]    = formObject.contract_number || '';
    newRow[OUTSOURCE_COL['Contract Duration'] - 1]  = formObject.contract_duration || '';
    newRow[OUTSOURCE_COL['Salary'] - 1]             = Number(formObject.salary) || 0;
    newRow[OUTSOURCE_COL['Recruitment Source'] - 1] = formObject.recruitment_source || '';
    ...
    writeAuditLog_(outsourceId, 'Outsource Registered', 'Status', '-', 'Pending — ' + formObject.position);
    return 'Sukses';
  } catch (error) {
...
function approveOutsourceToEmployee(outsourceId, notes) {
  ...
  lock.waitLock(10000);
  try {
    var srcSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(OUTSOURCE_SHEET_NAME);
    ...
    for (var i = 1; i < data.length; i++) {
      if (String(data[i][OUTSOURCE_COL['Outsource ID'] - 1]) !== String(outsourceId)) continue;
      ...
      newRow[EMPLOYEE_COL['Full Name'] - 1]         = row[OUTSOURCE_COL['Full Name'] - 1];
      ...
      newRow[EMPLOYEE_COL['Join Date'] - 1]         = row[OUTSOURCE_COL['Join Date'] - 1];
      ...
      newRow[EMPLOYEE_COL['Birth Date'] - 1]        = row[OUTSOURCE_COL['Birth Date'] - 1];
      newRow[EMPLOYEE_COL['Age'] - 1]               = row[OUTSOURCE_COL['Age'] - 1];
      newRow[EMPLOYEE_COL['Gender'] - 1]            = row[OUTSOURCE_COL['Gender'] - 1];
      newRow[EMPLOYEE_COL['Marital Status'] - 1]    = row[OUTSOURCE_COL['Marital Status'] - 1];
      newRow[EMPLOYEE_COL['Address'] - 1]           = row[OUTSOURCE_COL['Address'] - 1];
      newRow[EMPLOYEE_COL['City'] - 1]              = row[OUTSOURCE_COL['City'] - 1];
      newRow[EMPLOYEE_COL['Education'] - 1]         = row[OUTSOURCE_COL['Education'] - 1];
      newRow[EMPLOYEE_COL['Work Experience'] - 1]   = row[OUTSOURCE_COL['Work Experience'] - 1];
      newRow[EMPLOYEE_COL['Department'] - 1]        = row[OUTSOURCE_COL['Department'] - 1];
      newRow[EMPLOYEE_COL['Division'] - 1]          = '';
      newRow[EMPLOYEE_COL['Branch'] - 1]            = '';
      newRow[EMPLOYEE_COL['Contract Duration'] - 1] = row[OUTSOURCE_COL['Contract Duration'] - 1];
      ...
      newRow[EMPLOYEE_COL['Outsource Vendor'] - 1]  = row[OUTSOURCE_COL['Vendor Company'] - 1];
      newRow[EMPLOYEE_COL['Contract Number'] - 1]   = row[OUTSOURCE_COL['Contract Number'] - 1];
      newRow[EMPLOYEE_COL['District'] - 1]          = '';
      newRow[EMPLOYEE_COL['Recruitment Source'] - 1]= row[OUTSOURCE_COL['Recruitment Source'] - 1];
      ...
      writeAuditLog_(outsourceId, 'Outsource Approved', 'Status', 'Pending', 'Approved → Employee ' + employeeId);
      return { success: true, outsourceId: outsourceId, employeeId: employeeId, newStatus: 'Approved' };
    }

    return { success: false, message: 'Outsource ID tidak ditemukan: ' + outsourceId };
  } catch (err) {
...
function rejectOutsource(outsourceId, reason) {
  ...
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      writeAuditLog_(outsourceId, 'Outsource Rejected', 'Status', 'Pending', 'Rejected — ' + (reason || '-'));
...
function getOutsourceList() {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    result.push({
      ...
      createdDate:        row[colIndex['Created Date']] instanceof Date
                            ? Utilities.formatDate(row[colIndex['Created Date']], 'GMT+7', 'dd/MM/yyyy HH:mm')
