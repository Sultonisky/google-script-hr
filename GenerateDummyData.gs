...
function generateAllDummyData() {
  ...
  try {
    ...
    for (var j = 0; j < records.length; j++) {
      var rec = records[j];
      try {
        writeAuditLog_(rec.recruitmentId, 'Created (Dummy Data)', '-', rec.status);
      } catch (e) {
        Logger.log('Audit log skip for ' + rec.recruitmentId + ': ' + e.message);
      }
    }
    
    Logger.log('SUCCESS: ' + records.length + ' dummy records inserted at rows ' + startRow + '-' + (startRow + records.length - 1));
...
function buildDummyRecords_() {
  ...
  // Status distribution
  var statusList = [];
  addMultiple_(statusList, 'Applied', 20);
  addMultiple_(statusList, 'Screening', 15);
  addMultiple_(statusList, 'HR Interview', 15);
  addMultiple_(statusList, 'User Interview', 12);
  addMultiple_(statusList, 'Offering', 8);
  addMultiple_(statusList, 'Accepted', 12);
  addMultiple_(statusList, 'Rejected', 15);
  addMultiple_(statusList, 'Hold', 2);
  addMultiple_(statusList, 'Blacklist', 1);
  ...
  // Position list
  var positions = [
    'IT Support', 'Network Engineer', 'Software Engineer',
    'Backend Developer', 'Frontend Developer', 'Fullstack Developer',
    'HR Staff', 'HR Recruiter', 'Finance Staff', 'Accounting Staff',
    'Digital Marketing', 'Graphic Designer', 'UI/UX Designer',
    'Sales Executive', 'Purchasing Staff', 'Warehouse Staff',
    'Admin', 'Customer Service'
  ];
  
  // Indonesian first names
  var maleFirstNames = [
    'Ahmad', 'Budi', 'Dedi', 'Eko', 'Fajar', 'Gilang', 'Hendra', 'Irfan',
    'Joko', 'Krisna', 'Luthfi', 'Muhammad', 'Nanda', 'Oki', 'Prasetyo',
    'Rizki', 'Satria', 'Taufik', 'Ujang', 'Wahyu', 'Yoga', 'Aditya',
    'Bagus', 'Cakra', 'Dimas', 'Elang', 'Farhan', 'Guntur', 'Hafiz',
    'Indra', 'Januar', 'Kurniawan', 'Lukman', 'Maulana', 'Nugroho'
  ];
  
  var femaleFirstNames = [
    'Ani', 'Bunga', 'Citra', 'Dewi', 'Eka', 'Fitri', 'Gita', 'Hana',
    'Indah', 'Juli', 'Kartika', 'Lestari', 'Maya', 'Nina', 'Oktavia',
    'Putri', 'Ratna', 'Sari', 'Tantri', 'Ulya', 'Wati', 'Yunita',
    'Ayu', 'Bening', 'Cempaka', 'Dian', 'Elsa', 'Fiona', 'Grace',
    'Hani', 'Intan', 'Julia', 'Kirana', 'Luna', 'Mega'
  ];
  
  var lastNames = [
    'Susanto', 'Wijaya', 'Pratama', 'Kurniawan', 'Setiawan', 'Saputra',
    'Hidayat', 'Santoso', 'Putra', 'Ardianto', 'Nugroho', 'Suryadi',
    'Wibowo', 'Rahman', 'Firmansyah', 'Suhendar', 'Gunawan', 'Hartono',
    'Budiman', 'Siregar', 'Tampubolon', 'Manurung', 'Purba', 'Simanjuntak',
    'Limbong', 'Togatorop', 'Sinaga', 'Nainggolan', 'Hutapea', 'Panggabean'
  ];
  ...
  // Companies
  var companies = [
    'PT Telkom Indonesia', 'PT Bank Mandiri', 'PT Pertamina', 'PT PLN',
    'PT Garuda Indonesia', 'PT Astra International', 'PT Unilever Indonesia',
    'PT Indofood Sukses Makmur', 'PT Telekomunikasi Seluler',
    'PT Matahari Putra Prima', 'PT Kalbe Farma', 'PT BRI',
    'PT BCA', 'PT Bank Negara Indonesia', 'PT Wijaya Karya',
    'PT Adhi Karya', 'PT Jasa Marga', 'PT Angkasa Pura',
    'PT Pos Indonesia', 'PT Kereta Api Indonesia',
    'PT Samsung Electronics Indonesia', 'PT Xiaomi Communications',
    'PT Gojek Indonesia', 'PT Tokopedia', 'PT Traveloka',
    'PT Shopee Indonesia', 'PT Grab Indonesia', 'PT Lazada Indonesia',
  ...
  var availableToJoin = [
    'Segera', '1 Minggu', '2 Minggu', '1 Bulan', '2 Bulan', '3 Bulan', 'Bisa Negosiasi'
  ];
  
  // Recruitment sources
  var recruitmentSources = [
    'JobStreet', 'LinkedIn', 'Indeed', 'Glassdoor', 'Instagram',
    'Website Perusahaan', 'Referensi Karyawan', 'Kampus / Career Fair',
    'Indeed', 'Loker.id', 'Karir.com'
  ];
  ...
  var streetNames = [
    ...
    'Jl. Ahmad Yani', 'Jl. Imam Bonjol', 'Jl. Hayam Wuruk', 'Jl. Gajah Mada',
    'Jl. Veteran', 'Jl. Pahlawan', 'Jl. Merdeka', 'Jl. Asia Afrika',
    'Jl. Pemuda', 'Jl. Kartini', 'Jl. Sisingamangaraja', 'Jl. Rasuna Said',
    'Jl. Kuningan', 'Jl. TB Simatupang', 'Jl. HR Rasuna Said',
    'Jl. M.H. Thamrin', 'Jl. Jenderal Soedirman'
  ];
  
  // Universities
  var universities = [
    'Universitas Indonesia', 'Institut Teknologi Bandung', 'Universitas Gadjah Mada',
    'Universitas Diponegoro', 'Institut Teknologi Sepuluh Nopember',
    'Universitas Airlangga', 'Universitas Padjadjaran', 'Universitas Hasanuddin',
    'Universitas Brawijaya', 'Universitas Sebelas Maret',
    'Universitas Negeri Jakarta', 'Universitas Negeri Yogyakarta',
    'Universitas Muhammadiyah Jakarta', 'Universitas Trisakti',
    'Universitas Mercu Buana', 'Universitas Bina Nusantara',
    'Politeknik Negeri Jakarta', 'Politeknik Negeri Bandung',
    'Universitas Pancasila', 'Universitas Tarumanagara'
  ];
  ...
  var majors = [
    ...
    'Desain Grafis', 'Desain Komunikasi Visual', 'Seni Rupa',
    'Psikologi', 'Hubungan Masyarakat', 'Ilmu Komunikasi',
  ...
  // Hold reasons
  var holdReasons = [
    'Kandidat minta penundaan', 'Posisi belum dibuka', 'Budget belum tersedia',
    'Menunggu hasil background check'
  ];
  
  // Blacklist reasons
  var blacklistReasons = [
    'Tidak hadir tanpa konfirmasi', 'Berdusta dalam aplikasi', 'Pelanggaran etika saat interview'
  ];
  ...
  for (var i = 0; i < 100; i++) {
    ...
    if (education === 'S1' || education === 'S2' || education === 'D3' || education === 'D4') {
    ...
    var cvLink = '';
    if (Math.random() > 0.3) {
      cvLink = 'https://drive.google.com/file/d/' + randomAlphanumeric_(15) + '/view';
    }
    ...
    createdDateObj.setHours(randomInt_(8, 17), randomInt_(0, 59), randomInt_(0, 59));
    var createdDateStr = Utilities.formatDate(createdDateObj, 'GMT+7', 'yyyy-MM-dd HH:mm:ss');
    
    // Updated At
    var updatedDateObj = new Date(createdDateObj.getTime() + randomInt_(0, daysAgo) * 86400000);
    updatedDateObj.setHours(randomInt_(8, 17), randomInt_(0, 59), randomInt_(0, 59));
    ...
    if (status === 'Accepted') {
      employeeId = 'EMP-' + today.getFullYear() + '-' + ('0000' + (i + 1)).slice(-4);
    }
...
function validateInsertedData() {
  ...
  var statuses = ['Applied', 'Screening', 'HR Interview', 'User Interview', 'Offering', 'Accepted', 'Rejected', 'Hold', 'Blacklist'];
